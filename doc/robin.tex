...
This technical note serves as a notice of the correct coefficients. It also provides a link to a public repository
...
\item The denominator in the outermost exponent ($C_{8}$) was changed from 0 to 1 due to $C_{8}$ throwing either a NaN or Inf when evaluated. This 
change was made because $C_{7}=0$ and thus, the exponential term need not be evaluated. This was pointed out by Phelps \& Berry \cite{nasa87762}.
\item Similarly, $C_{4}$ was changed from 0 to 1 to prevent division by zero, because
$C_{2}=C_{5}=0$ means that there is no need to evaluate the term in the parenthesis.
\item Parameters in $C_{1}$ where often multiplied by 0 in $C_{7}$ when they were supposed to be the resulting values. These were moved to $C_{6}$.
